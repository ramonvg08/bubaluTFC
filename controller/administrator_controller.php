...
if (isset($_POST["accion"])) {
    ...
    switch ($_POST["accion"]) {
        ...
        case "subirImagenNegocio":
            // ... (código de subida de imagen sin cambios, asumiendo que ya maneja errores JSON)
                ...
                if (in_array($fileExt, $allowedExtensions)) {
                    if ($fileError === 0) {
                        if ($fileSize < 5000000) { // Menos de 5MB
                            ...
                            // Asegúrate que la ruta base para DOCUMENT_ROOT sea correcta para tu entorno.
                            // Si el script está en /tfc/controller/ y las imágenes en /tfc/images/
                            // $_SERVER["DOCUMENT_ROOT"] podría no ser lo que esperas si el alias de Apache es /tfc
                            // Una ruta relativa desde el script podría ser más robusta: __DIR__ . "/../../images/"
                            $uploadDir = __DIR__ . "/../../images/"; 
                            if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
                            $fileDestination = $uploadDir . $fileNameNew;
                            $dbFilePath = "images/" . $fileNameNew; // Ruta para la BD
                            if (move_uploaded_file($fileTmpName, $fileDestination)) {
                                $result = $datos->update_business_image($id_business, $dbFilePath);
                                if ($result) {
                                    // Ruta para eliminar la imagen vieja
                                    $oldImageFullPath = __DIR__ . "/../../" . $currentImage;
                                    if ($currentImage && $currentImage != "images/default_business.png" && file_exists($oldImageFullPath)) {
